...
# ::TreeCtrl::MotionInItems --
#
# This procedure fires Enter/Leave events for each item visited.
#
# Arguments:
# w		The treectrl widget.
# args		x y coords if the pointer is in the window, or an empty list.

proc ::TreeCtrl::MotionInItems {w args} {
    variable Priv
    if {[llength $args]} {
	set x [lindex $args 0]
	set y [lindex $args 1]
...
proc ::TreeCtrl::ButtonPress1Header {w id x y {columnDragOffset 5}} {
	...
	if {[$w column cget $column -lock] ne "none"} return
    ...
    if {[lindex $action 1] eq "header" && $columnDragOffset == 0} {
	Motion1 $w $x $y 0
    }
}
...
proc ::TreeCtrl::Motion1 {w x y {columnDragOffset 5}} {
    variable Priv
    if {![info exists Priv(buttonMode)]} return
    if {![info exists Priv(column)]} return
	...
	header {
		...
		if {[$w column dragcget -enable] &&
		    (abs($Priv(columnDrag,x) - $x) >= $columnDragOffset)} {
	...
	dragColumnWait {
	    if {(abs($Priv(columnDrag,x) - $x) >= $columnDragOffset)} {
		$w column dragconfigure \
		    -imagecolumn $Priv(column) \
		    -imageoffset [expr {$x - $Priv(columnDrag,x)}]
		set Priv(buttonMode) dragColumn
...
proc ::TreeCtrl::Release1 {w x y} {
	...
	dragColumn - dragColumnWait {
		...
		if {$Priv(buttonMode) eq "dragColumn"} {
...
proc ::TreeCtrl::SelectionMotion {w item} {
	...
	extended {
		...
		if {[lsearch -integer $Priv(selection) $i] >= 0} {
		    lappend select $i
		}
...
proc ::TreeCtrl::Cancel w {
	...
	if {[lsearch -integer $Priv(selection) $first] >= 0} {
...
proc ::TreeCtrl::finishBindings {T} {
    foreach key {   Up Down Home End Left Right
		    Shift-Up Shift-Down Shift-Left Shift-Right
		    Control-Left Control-Right Control-Prior Control-Next Control-Home Control-End
		    Shift-Control-Home Shift-Control-End
		    KeyPress-space KeyPress-Select KeyPress-Escape KeyPress-plus KeyPress-minus
		    KeyPress-Return KeyPress-Prior KeyPress-Next
		    Shift-KeyPress-Select
		    Control-KeyPress-slash Control-KeyPress-backslash
		    Control-Shift-KeyPress-space
		    <Copy>
		    ButtonPress-1 ButtonPress-2 Control-ButtonPress-3 Shift-ButtonPress-1
		    Command-ButtonPress-1 Control-ButtonPress-1
		    Control-Button3-Motion Double-ButtonPress-1
					} {
	::bind $T <$key> {+ break }
    }
}

...
bind TreeCtrlFileList <Control-ButtonPress-1> {
    TreeCtrl::FileListButton1 %W %x %y toggle
    break
}
bind TreeCtrlFileList <Shift-ButtonPress-1> {
    TreeCtrl::FileListButton1 %W %x %y add
    break
}
bind TreeCtrlFileList <ButtonPress-1> {
    TreeCtrl::FileListButton1 %W %x %y set
    break
}
...
proc ::TreeCtrl::EntryOpen {T item column element} {
	...
	$T item element configure $item $column $element -text " "
    }
	...
	entry $e -borderwidth 1 -relief solid -highlightthickness 0 -selectforeground white
...
#   $T notify bind $e <Scroll> { TreeCtrl::EditClose %T entry 0 1 }
    ...
    if {$fullWidth} {
       ...
       set Priv(entry,$T,minwidth) $width
    } else {
       # Make the Entry as wide as the text plus "W" but keep it within the TreeCtrl borders
       set width [font measure $font ${text}W]
       set width [expr {$width + ($ebw + 1) * 2}]
       set Priv(entry,$T,minwidth) 0
    }
    scan [$T contentbox] "%d %d %d %d" left top right bottom
    if {$ex + $width > $right} {
	set width [expr {$right - $ex}]

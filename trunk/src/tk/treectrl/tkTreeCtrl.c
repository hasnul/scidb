	...
	)
{
	...
	tree->updateIndex			= 1;
	tree->prevColumnWidth	= 0;
	tree->prevTreeWidth		= 0;
	tree->prevVisWidth		= 0;
	...
	args = malloc(sizeof(Tcl_Obj*)*(objc - 2));
	...
	if (TreeConfigure(interp, tree, argc, args, TRUE) != TCL_OK) {
		Tk_DestroyWindow(tree->tkwin);
		free(args);
		return TCL_ERROR;
	}
	...
	)
{
	...
	static CONST char *commandName[] = {
		...
		"expand",
#endif
		"headerheight", "identify", "index", "item", "marquee", "notify",
#ifdef DEPRECATED
		"numcolumns", "numitems",
#endif
		"orphans",
	...
	enum {
		...
		COMMAND_EXPAND,
#endif
		COMMAND_HEADERHEIGHT, COMMAND_IDENTIFY, COMMAND_INDEX, COMMAND_ITEM,
		COMMAND_MARQUEE, COMMAND_NOTIFY,
#ifdef DEPRECATED
	...
	switch (index) {
		...
		case COMMAND_HEADERHEIGHT: {
			Tcl_SetObjResult(interp, Tcl_NewIntObj(Tree_HeaderHeight(tree)));
			break;
		}
	...
	)
{
	...
	switch (eventPtr->type) {
		...
		case ConfigureNotify: {
			if (!tree->keepUserWidth && tree->prevWidth != Tk_Width(tree->tkwin))
				Tree_ResetColumns(tree);
...
#define WIN7
	...
	XImage *ximage;
	XColor *xcolors;
	unsigned long red_shift = 0, green_shift = 0, blue_shift = 0;
	int i, ncolors;
	int separated = 0;
#endif
...
# endif
	...
	if (Tcl_PkgProvide(interp, "tktreectrl", PACKAGE_PATCHLEVEL) != TCL_OK) {
		return TCL_ERROR;
	}

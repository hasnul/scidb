...
{
	int varno = findVariationNo(move);
	return varno == -1 ? 0 : &m_lines[varno];
}
...
inline
int
Engine::Concrete::findVariationNo(db::Move const& move) const
{
	return m_engine->findVariationNo(move);
}

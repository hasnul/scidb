	...
	)
{
	...
	column->textLayout = TextLayout_Compute(tkfont, NULL, text,
	...
	if (widthForText > 0) {
		...
		if (column->textLayout != NULL) {
			...
			if (widthForText >= column->textWidth) {
				...
				partText.height = layout->fm.linespace;
				layout->bytesThatFit = Tree_Ellipsis(layout->tkfont, NULL,
	...
	)
{
	...
	if (tree->useTheme && TreeTheme_GetHeaderFixedHeight(tree, &column->neededHeight) == TCL_OK) {
		return column->neededHeight;
	}
#endif
...
static void
RefreshWidthObj(TreeColumn column)
{
	if (column->widthObj != NULL)
	{
		Tcl_DecrRefCount(column->widthObj);
		column->hasWidthObj = 1;
	}

	column->widthObj = Tcl_NewIntObj(column->width);
	Tcl_IncrRefCount(column->widthObj);
	column->neededWidth = -1;
}


static void
ResetColumn(TreeColumn column)
{
	if (column->visible && !column->hasWidthObj && column->widthObj) {
		Tcl_DecrRefCount(column->widthObj);
...
{
	if (TreeColumn_MinWidth(column) > 0)
		column->width = TreeColumn_MinWidth(column);
	else if (TreeColumn_FixedWidth(column) >= 0)
		column->width = TreeColumn_FixedWidth(column);
	else
		column->width = 0;

	if (expandOnly)
		column->width = MAX(TreeColumn_UseWidth(column), column->width);

	column->widthOfItems = -1;
	if (!shrinkOnly) {
		column->width = MAX(TreeColumn_WidthOfItems(column), column->width);
	} else if (column->width) {
		column->width = MIN(TreeColumn_WidthOfItems(column), column->width);
	} else {
		column->width = TreeColumn_WidthOfItems(column);
	}
...
ExpandColumn(TreeColumn column, int fullWidth)
{
	...
	if (fullWidth)
		treeWidth = MAX(tree->prevVisWidth, treeWidth);

	spaceRemaining = treeWidth - totalWidth;

	if (spaceRemaining > 0) {
		column->width += spaceRemaining;
		RefreshWidthObj(column);
	...
	)
{
	...
	static CONST char *commandNames[] = {
		...
		"index",
#endif
		"list", "minimumwidth", "move", "neededwidth", "optimize", "order",
		"squeeze", "tag", "width", (char *) NULL
	};
	enum {
		...
		COMMAND_INDEX,
#endif
		COMMAND_LIST, COMMAND_MINIMUMWIDTH, COMMAND_MOVE, COMMAND_NEEDEDWIDTH,
		COMMAND_OPTIMZE, COMMAND_ORDER, COMMAND_SQUEEZE, COMMAND_TAG,
		COMMAND_WIDTH
	};
	int index;
	TreeColumnList columns;
	...
	switch (index) {
		...
		case COMMAND_CREATE: {
			...
			/* HACK (gc) */
			tree->columnTail->borderWidth = column->borderWidth;
...
doneDELETE:
		...
		case COMMAND_ELLIPSIS: {
			...
			if (TreeColumn_FromObj(tree, objv[3], &column,
						CFO_NOT_NULL) != TCL_OK)
				return TCL_ERROR;
			Tcl_SetObjResult(interp, Tcl_NewBooleanObj(column->ellipsis));
			break;
		}
		...
		case COMMAND_MINIMUMWIDTH: {
			...
			if (!column->visible)
				width = 0;
			else if (TreeColumn_MinWidth(column) > 0)
				width = TreeColumn_MinWidth(column);
			else if (TreeColumn_FixedWidth(column) >= 0)
				width = TreeColumn_FixedWidth(column);
			else
				width = 0;
		...
		case COMMAND_EXPAND: {
			...
			if (objc != 4 && objc != 5) {
				Tcl_WrongNumArgs(interp, 3, objv, "column ?full-width-flag?");
				return TCL_ERROR;
			}
			if (TreeColumn_FromObj(tree, objv[3], &column, CFO_NOT_NULL) != TCL_OK)
				return TCL_ERROR;
			if (objc == 5 && Tcl_GetBooleanFromObj(tree->interp, objv[4], &fullWidth) != TCL_OK)
				return TCL_ERROR;
		...
		case COMMAND_FIT: {
			...
			if (length > 0) {
				for (i = 0; i < length; ++i) {
					...
					if (Tcl_ListObjIndex(tree->interp, objv[3], i, &obj) != TCL_OK)
						return TCL_ERROR;

					if (TreeColumn_FromObj(tree, obj, &column, CFO_NOT_NULL) != TCL_OK)
						return TCL_ERROR;

					if (column->visible)
						OptimizeColumn(column, TRUE, FALSE);
				}
			} else if (objc == 3) {
				column = tree->columns;
			...
			/* recompute layout */
			tree->widthOfColumns = tree->widthOfColumnsLeft = tree->widthOfColumnsRight = -1;
			Tree_DInfoChanged(tree, DINFO_REDO_COLUMN_WIDTH);
			(void)Tree_WidthOfColumns(tree);
		...
		case COMMAND_OPTIMZE: {
			...
			if (length > 0) {
				for (i = 0; i < length; ++i) {
					...
					if (column->visible)
						OptimizeColumn(column, FALSE, FALSE);
				}
			} else if (objc == 3) {
				column = tree->columns;
		...
		case COMMAND_SQUEEZE: {
			...
			if (length > 0) {
				int alwaysSqueeze = tree->alwaysSqueeze;
				...
				for (i = 0; i < length; ++i) {
					...
					if (column->visible && column->squeeze && column->widthObj) {
						Tcl_DecrRefCount(column->widthObj);
						column->widthObj = NULL;
						column->hasWidthObj = 1;
					}
				...
				tree->prevVisWidth = 0x7fffffff;
				tree->widthOfColumns = tree->widthOfColumnsLeft = tree->widthOfColumnsRight = -1;
				Tree_DInfoChanged(tree, DINFO_REDO_COLUMN_WIDTH);

				/* recompute layout */
				tree->alwaysSqueeze = 1;
				(void)Tree_WidthOfColumns(tree);
				tree->alwaysSqueeze = alwaysSqueeze;
				...
				for (i = 0; i < length; ++i) {
					...
					if (column->visible && column->squeeze) {
						column->width = column->useWidth;
						if (column->hasWidthObj) {
							column->widthObj = Tcl_NewIntObj(column->width);
							Tcl_IncrRefCount(column->widthObj);
							column->hasWidthObj = 0;
						}
...
Column_Draw(
	...
	TreeDrawable td,			/* Where to draw. */
	int x, int y,				/* Top-left corner of the column's header. */
	int visIndex,               /* 0-based index in the list of visible columns. */
 ...
 * Tree_IsSteadyColumn --
 *
 *		Returns whether this column is steady,
 *
 * Results:
 *		Returns 1 if column is steady, returns 0 otherwise.
 *
 * Side effects:
 *		None.
 *
	...
	)
{
	...
	while (column != NULL && column->lock == first->lock) {
		if (column->visible) {
			if (column->widthObj != NULL) {
				...
				if (column->squeeze)
					numSqueeze += MAX(column->weight, 1);
			}
	...
	if (visWidth < totalWidth &&
		...
		int w = MIN(spaceRemaining, tree->columnTail->width);
		tree->columnTail->width -= w;

		/* firstly squeeze all columns which are larger than needed */
		column = first;
		while (column != NULL && column->lock == first->lock) {
			if (column->visible && column->squeeze && column->widthObj == NULL) {
				...
				if (TreeColumn_MaxWidth(column) != -1)
					width = MIN(width, TreeColumn_MaxWidth(column));
				column->overplus = column->useWidth - width;
				if (column->overplus > 0) {
					overplus += column->overplus;
					++numColumns;
				}
			} else {
				column->overplus = 0;
		...
		overplus = MIN(overplus, spaceRemaining);
		while (overplus > 0) {
			int each = (overplus >= numColumns) ? overplus / numColumns : 1;
			numColumns = 0;
			column = first;
			while (column != NULL && column->lock == first->lock) {
				if (column->overplus > 0) {
					int sub = MIN(overplus, MIN(column->overplus, each));
					column->useWidth -= sub;
					column->overplus -= sub;
					overplus -= sub;
					spaceRemaining -= sub;
					if (!spaceRemaining || !overplus) break;
					if (column->overplus > 0)
						numColumns++;
				}
				column = column->next;
		...
		/* secondly squeeze all columns regardlessly */
			...
			while (column != NULL && column->lock == first->lock) {
				if (column->visible && column->squeeze && (column->widthObj == NULL)) {
					...
					if (column->useWidth > min) {
						int sub = MIN(spaceRemaining, MIN(each, column->useWidth - min));
						column->useWidth -= sub;
						spaceRemaining -= sub;
	...
	if (visWidth > totalWidth &&
			...
			(tree->prevTreeWidth < Tk_Width(tree->tkwin) ||
				...
				tree->prevColumnWidth < totalWidth ||
				tree->prevColumnCount > columnCount)) {
		if (tree->columnTail->width <= 2)
			tree->columnTail->width = 0;
		int spaceRemaining = visWidth - totalWidth - MAX(0, tree->columnTail->width);
 ...
 * Tree_ResetColumns --
 *
 *		Reset width info according to column weight.
 *
 * Results:
 *		None,
 *
	...
	)
{
	...
	while (column != NULL) {
		if (column->weight > 0 && column->widthObj) {
			Tcl_GetIntFromObj(NULL, column->widthObj, &column->width);
			Tcl_DecrRefCount(column->widthObj);
			column->widthObj = NULL;
		}
		column = column->next;

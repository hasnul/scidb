	...
	)
{
	...
	if (csM & CS_LAYOUT) {
		...
		{
			TreeColumn treeColumn = Tree_FindColumn(tree, columnIndex);
			if (!Tree_IsSteadyColumn(treeColumn)) {
				Tree_InvalidateColumnWidth(tree, treeColumn);
				TreeItemColumn_InvalidateSize(tree, column);
			}
...
void Tree_ElementIterateChanged(TreeIterate iter_, int mask)
{
	...
	if (mask & CS_LAYOUT) {
		TreeColumn treeColumn = Tree_FindColumn(iter->tree, iter->columnIndex);
		...
		if (!Tree_IsSteadyColumn(treeColumn)) {
			Tree_InvalidateColumnWidth(iter->tree, treeColumn);

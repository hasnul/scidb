...
{
	ElementTextVar *etv = DynamicOption_FindData(elemX->header.options, DOID_TEXT_VAR);
...
static void DeleteProcText(TreeElementArgs *args)
{
	...
	if (etl2 != NULL && etl2->layout != NULL)
		TextLayout_Free(etl2->layout);
	DynamicOption_Free1(tree, &elem->options, DOID_TEXT_LAYOUT2, sizeof(ElementTextLayout2));
#ifdef TEXTVAR
...
int TreeElement_Init(Tcl_Interp *interp)
{
	...
	DynamicCO_Init(treeElemTypeImage.optionSpecs, "-height",
		DOID_TEXT_VAR, sizeof(ElementImageSize),

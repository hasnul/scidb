	...
	)
{
	...
	if (tkfont2)
	{
		...
		*pixels = 0;
		...
		if (bytesThatFit == 0 && (flags & TK_AT_LEAST_ONE))
		{
			Tk_Font font;
			int length;
	...
	{
		bytesThatFit = Tk_MeasureChars(tkfont, string, numBytes, maxPixels, flags, pixels);
	}
...
Tree_TextWidth(Tk_Font tkfont, Tk_Font tkfont2, CONST char* string, int numBytes)
{
	...
	{
		width = Tk_TextWidth(tkfont, string, numBytes);
	}
...
if (*objPtrPtr) {
	...
	dbwin("DynamicCO_Free free object NULL\n");
			...
			if (*objPtrPtr != NULL) {
				Tcl_DecrRefCount(*objPtrPtr);
				*objPtrPtr = NULL;	/* it's a hack, but it prevents a memory error */
			}
		}
	}
}
